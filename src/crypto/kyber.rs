...
impl Kyber {
    ...
    pub fn new() -> Self {
        Self::new_with_level(KyberSecurityLevel::Kyber768)
    }
    ...
    pub fn with_key_rotation_interval(mut self, interval: core::time::Duration) -> Self {
        self.rotation.key_rotation_interval = interval;
        self
    }
}
...
impl PqcKEM for Kyber {
    ...
    fn generate_keypair(&self) -> Result<(Vec<u8>, Vec<u8>), Self::Error> {
        kyber_generate_keypair(self.security_level)
    }

    fn encapsulate(&self, pk: &[u8]) -> Result<(Vec<u8>, Vec<u8>), Self::Error> {
        kyber_encapsulate(self.security_level, pk)
    }

    fn decapsulate(&self, sk: &[u8], ct: &[u8]) -> Result<Vec<u8>, Self::Error> {
        kyber_decapsulate(self.security_level, sk, ct)
    }
}
...
#[cfg(not(feature = "kyber-pqclean"))]
fn kyber_generate_keypair(
    _security_level: KyberSecurityLevel,
) -> Result<(Vec<u8>, Vec<u8>), Error> {
    Err(Error::CryptoError(
        "Kyber backend unavailable: enable feature `kyber-pqclean`".into(),
    ))
}
