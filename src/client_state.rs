...
impl PqcClient<Unprovisioned> {
    ...
    ) -> Result<PqcClient<Provisioned>> {
        if operational_cert.is_empty() {
            return Err(crate::error::Error::ClientError(
                "Empty Operational Certificate".into(),
            ));
        }

        Ok(PqcClient {
            state: Provisioned { operational_cert },
...
impl PqcClient<Provisioned> {
    ...
    pub fn generate_connect_request(&self) -> Result<(Vec<u8>, Vec<u8>)> {
        ...
        let x_sk = x25519_dalek::StaticSecret::random_from_rng(rand_core::OsRng);
    ...
    pub fn complete_connection(
        ...
        if server_hello.len() < 32 {
            return Err(crate::error::Error::CryptoError(
                "Invalid ServerHello length".into(),
            ));
        }
        ...
        combiner
            .expand(&x_secret.to_bytes(), &mut final_secret)
            .map_err(|_| crate::error::Error::CryptoError("Hybrid KDF Expand Fail".into()))?;

        // 6. Initialize Ratchet
        let ratchet =
            RatchetSession::initialize(final_secret, Some(self.provider.kem_public_key()));
...
impl PqcClient<Connected> {
    ...
    pub fn seal_state(&self) -> Result<()> {
        let serialized = serde_json::to_vec(&self.state).map_err(|e| {
            crate::error::Error::ClientError(format!("Seal Serialization Fail: {}", e))
        })?;
    ...
    pub fn load_from_sealed(provider: Box<dyn SecurityProvider>, session_id: &str) -> Result<Self> {
        ...
        let state: Connected = serde_json::from_slice(&data).map_err(|e| {
            crate::error::Error::ClientError(format!("Unseal Deserialization Fail: {}", e))
        })?;
